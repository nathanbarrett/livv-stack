...
Prism Server allows you to expose your Prism-powered AI models through a standardized API. This makes it easy to integrate your custom AI solutions into various applications, including chat interfaces and other tools that support OpenAI-compatible APIs.
...
Make sure Prism Server is enabled in your `config/prism.php` file:
...
Register your Prism models in a service provider, such as `AppServiceProvider`:
...
Generate text using your registered Prism models:
...
Get a list of all registered Prism models:
...
Prism Server works seamlessly with OpenAI-compatible chat interfaces like [Open WebUI](https://openwebui.com/):
...
Add middleware to the Prism Server routes by setting the `middleware` option:
